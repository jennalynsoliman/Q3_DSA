...
if (userType === "SELLER") {  //if the user choose the seller it will ask for the username and password
  ...
  if (authenticateSeller(username, password, sellers)) {  //after logging the user passwords and username this will appear on the program and will asked on what do u want next
    ...
    } else if (action === "REMOVE") { //when the user choose remove
      const category = prompt("Which category?"); // this will print and asked which category to be removed
      do {
  ...
  //on this empty array part this will be going to store the customer's cart and order history
  const cart = []; 
  const orderHistory = []; 
...
function displayMenu(items) {
  ...
  for (const category in items) {
    // on this part the items will be sorted by price (ascending) before displaying
    items[category].sort((a, b) => a.price - b.price);
    console.log(`- ${category}`);
    items[category].forEach(item => {
      console.log(`  - ${item.name} (₱${item.price})`); //this prints the name and the price of the item in the program
    });
  }
}
...
function addToCart(cart, category, item, quantity, items) {
  ...
  if (selectedItem) {
    ...
    console.log("Item not found."); // this prints on the program when item is not found or not in the list
  }
}